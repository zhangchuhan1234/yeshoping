      ...
      <van-tab v-for="item in imagedata2" :key="item.id" :title="item.name">
        <van-tabs @click-tab="onClickTab">
          <van-tab
            ...
            :key="item.sub[index2].id"
            :title="item2.name"
            :name="item.sub[index2].id"
          >
            <van-card
              v-for="(item3, index3) in shoppinglist"
              :key="item3.id"
              :title="item3.name"
              :thumb="item3.image"
            >
              ...
              <template #footer>
                ...
                <van-button size="mini" @click="cuanc(item3)" v-show="xian(item3)"
                  >加入购物车</van-button
                >
                <van-button size="mini" @click="quxiao(item3)" v-show="!xian(item3)"
                  >取消加入</van-button
                >
              </template>
            </van-card>
...
async function b(a, b) {
  ...
  totle -= imagedata2.value[a].sub.length
  totle += b
  gvc.value[totle] = !gvc.value[totle].push(imagedata2.value[a].sub[b])
